      ...
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          ...
          <a
            ...
            className="px-8 py-4 bg-white/10 hover:bg-[var(--color-accent)] text-white hover:text-[#020502] font-bold tracking-widest text-sm uppercase rounded-full border border-[var(--color-accent)]/30 transition-all flex items-center gap-3 shrink-0"
          >
            <FaInstagram className="w-5 h-5" /> @deedishomefoods
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-6 md:h-[700px]">
          {posts.map((post, i) => (
            <motion.a
              ...
              className={`relative overflow-hidden rounded-[2.5rem] group cursor-pointer border border-white/5 block ${post.span}`}
            >
